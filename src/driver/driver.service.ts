...
export class DriverService {
  ...
  async acceptTrip(tripId: string, driverId: string): Promise<Trip> {
    const trip = await this.tripService.updatetrip(tripId, { driverId, status: TripStatus.ACCEPTED })
      if (!trip) {
  ...
  async rejectTrip(tripId: string, driverId: string) {
    ...
    const updatedTrip = this.mapClient.findDriver(trip).then(async (driver) => {
      const updatedTrip = await this.updateTrip(trip._id, {driverId: driver.driverId, status: TripStatus.WAITING});
      ...
      this.notificationClient.sendTripNotificationToCustomer(updatedTrip.customerId, updatedTrip);
      return updatedTrip;
    }).catch(err => {
              throw new TripException(
  ...
  async updateTripStatus(tripId: string, status: TripStatus): Promise<Trip> {
    ...
    if (!trip) {
      ...
      this.notificationClient.sendTripNotificationToCustomer(trip.customerId, trip);
      return trip;
  }
